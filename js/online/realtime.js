...
(function(){
  ...
  function applySharedState(shared){
    ...
    try{
      ...
      // The map image and every canvas must use exactly the same WORLD dimensions.
      // Never rely on CSS/display dimensions here — only on the image's natural size
      // or the dimensions saved in the shared state.
      const resizeWorldLayers = () => {
        ...
        ['grid-canvas','draw-canvas','fog-canvas','shapes-back-canvas','shapes-canvas'].forEach(id=>{
          ...
          if(c){ c.width=w; c.height=h; c.style.width=w+'px'; c.style.height=h+'px'; }
        });
      ...
      if(map.src){
        const sameImage = img && img.getAttribute('src') === map.src;
        if(img && !sameImage){
          img.onload=()=>{
            state.map.width=img.naturalWidth || Number(map.width) || 0;
            state.map.height=img.naturalHeight || Number(map.height) || 0;
            resizeWorldLayers();
          };
          img.onerror=()=>toastSafe('Не удалось загрузить карту','error');
          img.src=map.src;
        }else{
          // Image is already loaded (or browser restored it from cache).
          state.map.width=Number(map.width)||img?.naturalWidth||0;
          state.map.height=Number(map.height)||img?.naturalHeight||0;
          resizeWorldLayers();
        }
      }else{
  ...
  online.handleConnected=function(ws,meta){
    ...
    // Do NOT mark the current local state as already synchronized. The room may be
    // empty on the server (for example when the master loaded a map before joining),
    // in which case the first empty snapshot must cause us to publish our local state.
    online.lastSignature='';
  ...
  online.handleMessage=function(m){
    if(m.type==='state_snapshot'){
      ...
      const remote = m.state || {};
      const hasRemoteState = !!(
        remote.map?.src ||
        (Array.isArray(remote.tokens) && remote.tokens.length) ||
        (Array.isArray(remote.shapes) && remote.shapes.length) ||
        (Array.isArray(remote.characters) && remote.characters.length) ||
        (Array.isArray(remote.monsters) && remote.monsters.length) ||
        (Array.isArray(remote.initiative) && remote.initiative.length)
      );

      // Empty room: keep the local state of the client that already has the map/game
      // and publish it. Non-empty room: server is authoritative and we apply it.
      if(!hasRemoteState){
        online.lastSignature='';
        publishState('initial_room_state');
      }else{
