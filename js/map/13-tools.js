...
function setTool(tool) {
  if (window.dndOnlineCan && !window.dndOnlineCan('master') && ['brush','eraser','fog','text','measure','shape'].includes(tool)) {
    toast('Инструменты карты доступны только мастеру', 'error');
    tool = 'select';
  }
  state.tool = tool;
...
function toggleGrid() {
  if (window.dndOnlineCan && !window.dndOnlineCan('master')) { toast('Настройки карты доступны только мастеру', 'error'); return; }
